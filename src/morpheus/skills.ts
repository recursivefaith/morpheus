 ...
 * @returns 
 */
export default function mixinSkills(baseClass: typeof MorpheusCore) {
  return class extends baseClass {
    /**
     * Gets all the skills in settings.
     */
    getSkills () {
      const files = this.app.vault.getFiles().filter(file => file.path.startsWith(this.settings.skillFolder))
      const skills = []
